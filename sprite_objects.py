...
class Sprites:
    def __init__(self):
        self.sprite_parameters = {
            ...
            'sprite_flame': {
                'sprite': pygame.image.load('sprites/arrow/base/0.png').convert_alpha(),
                'viewing_angles': None,
                'shift': 0.7,
                'scale': 0.6,
                'animation': deque(
                    [pygame.image.load(f'sprites/arrow/anim/{i}.png').convert_alpha() for i in range(8)]),
                'animation_dist': 800,
                'animation_speed': 5,
                'blocked': False,
            },
...
class SpriteObject:
    ...
    def object_locate(self, player):
        ...
        if 0 <= fake_ray <= FAKE_RAYS_RANGE and distance_to_sprite > 30:
            ...
            shift = half_proj_height * self.shift * 2.5
